...
object Harvester {
  ...
  def props(datasetContext: DatasetContext, timeOut: Long) = Props(new Harvester(timeOut, datasetContext))
}

class Harvester(val timeout: Long, val datasetContext: DatasetContext) extends Actor with Harvesting {
  ...
  def receive = {
    ...
    case HarvestAdLib(strategy, url, database, search) => actorWork(context) {
      ...
      val futurePage = fetchAdLibPage(timeout, strategy, url, database, search)
    ...
    case AdLibHarvestPage(records, url, database, search, strategy, diagnostic) => actorWork(context) {
      ...
      if (diagnostic.isLast) {
        ...
        val futurePage = fetchAdLibPage(timeout, strategy, url, database, search, Some(diagnostic))
    ...
    case HarvestPMH(strategy: HarvestStrategy, raw_url, set, prefix) => actorWork(context) {
      ...
      val futurePage = fetchPMHPage(timeout, strategy, url, set, prefix)
    ...
    case PMHHarvestPage(records, url, set, prefix, total, strategy, resumptionToken) => actorWork(context) {
      ...
      resumptionToken.map { token =>
        ...
        val futurePage = fetchPMHPage(timeout, strategy, url, set, prefix, resumptionToken)
