...
import web.DatasetUpdateMonitorActor.RemoveWatcher

import scala.concurrent.{ExecutionContext, Future}
import scala.concurrent.duration._

class WebSocketController(val cacheApi: CacheApi,
                          datasetUpdateMonitorActor: ActorRef,
  ...
  def dataset: WebSocket = WebSocket.acceptOrResult[JsValue, JsValue] {
  ...
  def wsFutureFlow(request: RequestHeader): Future[Flow[JsValue, JsValue, NotUsed]] = {
    ...
    val datasetSocketActorFuture = createUserSocketActor(request.id.toString, webSocketOut)

    // Once we have an actor available, create a flow...
    datasetSocketActorFuture.map { userActor =>
  ...
  def createWebSocketFlow(webSocketIn: Publisher[JsValue], userActor: ActorRef): Flow[JsValue, JsValue, NotUsed] = {
    ...
    val flowWatch: Flow[JsValue, JsValue, NotUsed] = flow.watchTermination() { (_, termination) =>
      termination.foreach { done =>
        ...
        datasetUpdateMonitorActor.tell(RemoveWatcher, userActor)
  ...
  /**
    ...
    */
  def createUserSocketActor(name: String, webSocketOut: ActorRef): Future[ActorRef] = {
