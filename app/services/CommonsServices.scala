...
class CommonsServices(commonsHost: String, orgId: String, apiToken: String, node: String) extends Status {
    ...
    try {
      ...
      case timeout: TimeoutException =>
        ...
        if (retry < 3) {
          ...
          println("Still getting a timeout error while contacting CultureCommons, after %s attempts".format(retry), timeout)
          None
        }
      case t: Throwable =>
        println("Error contacting CultureCommons", t)
        None
    }
  }
  ...
  def connect(userName: String, password: String): Boolean = {
    ...
    get("/user/authenticate/" + URLEncoder.encode(hash, "utf-8")).exists(response => response.status == OK)
  }
  ...
  def isAccountActive(email: String): Boolean = {
    get("/user/active/" + email).exists(response => response.status == OK)
  }
  ...
  def resetPassword(resetToken: String, newPassword: String): Boolean = {
    post("/user/changePassword/" + resetToken, "newPassword" -> MissingLibs.passwordHash(newPassword, MissingLibs.HashType.SHA512)).exists(response => response.status == OK)
  }
  ...
  def updateUserProfile(userName: String, profile: UserProfile): Boolean = {
    postWithBody("/user/profile/" + userName, Json.toJson[UserProfile](profile)).exists(response => response.status == OK)
  }
  ...
  def isAdmin(orgId: String, userName: String): Boolean = {
    get("/organization/" + orgId + "/admin/" + userName).exists(response => response.status == OK)
  }
  ...
  def addAdmin(orgId: String, userName: String): Boolean = {
    post("/organization/" + orgId + "/admin/" + userName).exists(response => response.status == OK)
  }

  def removeAdmin(orgId: String, userName: String): Boolean = {
    delete("/organization/" + orgId + "/admin/" + userName).exists(response => response.status == OK)
  }
  ...
  def getName(orgId: String, language: String): Option[String] = {
    get("/organization/" + orgId).map {
      response =>
        if (response.status == OK) {
          Json.fromJson[OrganizationProfile](response.json).asOpt.map(_.name.getOrElse(language, ""))
        } else {
          None
        }
    }.getOrElse(None)
