...
/** Dataset with an active (non-completed) operation — for startup recovery. */
case class ActiveOperationRecord(
    spec: String,
    currentOperation: String,
    operationTrigger: Option[String],
    operationStart: Option[Instant]
)

/** Combined dataset + state + harvest config + mapping config for batch listing.
  * Avoids N+1 queries when listing all datasets.
  */
case class DatasetWithDetails(
...
trait DatasetRepository {
  ...
  def listActiveDatasetsWithDetails(orgId: String): List[DatasetWithDetails]
